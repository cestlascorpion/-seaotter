...
pthread_mutex_t RingLog::_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t RingLog::_cond = PTHREAD_COND_INITIALIZER;

RingLog *RingLog::_ins = nullptr;
pthread_once_t RingLog::_once = PTHREAD_ONCE_INIT;
uint32_t RingLog::_one_buff_len = 30 * 1024 * 1024; // 30MB
...
void RingLog::init_path(const char *log_dir, const char *prog_name, int level) {
...
void RingLog::try_append(const char *lvl, const char *format, ...) {
    int ms;
    ...
    // int prev_len = snprintf(log_line, LOG_LEN_LIMIT, "%s[%d-%02d-%02d %02d:%02d:%02d.%03d]", lvl, _tm.year, _tm.mon,
    // _tm.day, _tm.hour, _tm.min, _tm.sec, ms);
    ...
    int main_len = vsnprintf(log_line + prev_len, size_t(LOG_LEN_LIMIT - prev_len), format, arg_ptr);

    va_end(arg_ptr);

    auto len = uint32_t(prev_len + main_len);
    ...
    if (_curr_buf->status == cell_buffer::FREE && _curr_buf->avail_len() >= len) {
        ...
        if (_curr_buf->status == cell_buffer::FREE) {
            ...
            if (next_buf->status == cell_buffer::FULL) {
                ...
                if (_one_buff_len * (uint32_t(_buff_cnt) + 1u) > MEM_USE_LIMIT) {
...
bool RingLog::decis_file(int year, int mon, int day) {
    // TODO: 是根据日志消息的时间写时间？还是自主写时间？  I select 自主写时间
    if (!_env_ok) {
        if (_fp)
            fclose(_fp);
        _fp = fopen("/dev/nullptr", "w");
        return _fp != nullptr;
    }
    if (!_fp) {
