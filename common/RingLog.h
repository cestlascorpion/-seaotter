...
struct utc_timer {
    utc_timer() {
        ...
        _sys_acc_sec = (uint64_t)tv.tv_sec;
...
public:
    ...
    void persist(FILE *fp) {
        auto wt_len = (uint32_t)fwrite(_data, 1, _used_len, fp);
        if (wt_len != _used_len) {
...
public:
    ...
    static RingLog *ins() {
        pthread_once(&_once, RingLog::init);
        return _ins;
    }
...
private:
    ...
    RingLog(const RingLog &);
    const RingLog &operator=(const RingLog &);
